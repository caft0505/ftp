...
int analyser::checkout()
{
    ...
    return (checkcode == m_pack[m_pack.size() - 1] );
}
...
u_short analyser::get_pack_size()
{
    ...
    u_short res =  (parse_short(buff, sizeof(short)));

    // It is data's size ,
    // pack-size = type-size of data-size +
    //             data-size + pack-type + checkcode
    return res + 2 + 1 + 1;
}
...
int analyser::get_file_name(char * file_name, int len) 
{
    ...
    u_int file_name_len = get_fname_len();
...
u_int analyser::get_file_size()
{
    ...
    u_char buff[sizeof(int)] = {0};
    int start_index = 1 + 1 + get_fname_len();
...
int analyser::get_file_mode()
{
    ...
    u_char buff[sizeof(int)] = {0};
    int start_index = 1 + 1 + get_fname_len() + 4;
