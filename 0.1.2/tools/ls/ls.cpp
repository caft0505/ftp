        ...
        struct stat statbuf, char style[], Node *head)
{
    ...
    {
        if(-1 == get_length(statbuf, type, style))
        {
            return -1;
        }
    }
        ...
        char style[], char tmpStore[])
{
    ...
    if(type & _H)
    {
        ...
        {
            ++unit_i;
            size = statbuf.st_size / _pow(ONE_K, unit_i);
        }
...
float _pow(float sdata, int exponent)
{
    ...
    return (sdata * _pow(sdata, --exponent));
}
...
int get_length(struct stat statbuf, arg_type type, char maxStyle[])
{
    ...
    if(type & _H)
    {
        ...
        {
            ++i;
            size = statbuf.st_size / _pow(ONE_K, i);
        }
...
int get_l_style(const char *filePath, arg_type type, char style[])
{
    ...
    while(NULL != (entry = readdir(dp)))
    {
        ...
        if(-1 == get_length(statbuf, type, maxStyle))
        {
            closedir(dp);
            return -1;
        }
...
bool show_dir(const char *dir, arg_type type, Node *head)
{
    ...
    if(type & _L)
    {
        if(-1 == get_l_style(dir, type, style))
        {
            return -1;
        }
    }
